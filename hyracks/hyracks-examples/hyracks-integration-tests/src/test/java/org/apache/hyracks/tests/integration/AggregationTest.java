...
public class AggregationTest extends AbstractIntegrationTest {
    ...
    final IFileSplitProvider splitProvider2 = new ConstantFileSplitProvider(
            new FileSplit[] { new FileSplit(NC2_ID, new FileReference(new File("data/tpch0.001/orderslmdd.tbl"))) });

    final RecordDescriptor desc2 = new RecordDescriptor(new ISerializerDeserializer[] {
            new UTF8StringSerializerDeserializer(), new UTF8StringSerializerDeserializer(),
            new UTF8StringSerializerDeserializer(), new UTF8StringSerializerDeserializer(),
            new UTF8StringSerializerDeserializer(), new UTF8StringSerializerDeserializer(),
            new UTF8StringSerializerDeserializer(), new UTF8StringSerializerDeserializer(),
    ...
    final ITupleParserFactory tupleParserFactory2 = new DelimitedDataTupleParserFactory(new IValueParserFactory[] {
    ...
    public void singleKeySumPreClusterGroupTest() throws Exception {
        ...
        runTest(spec);
        File temp=new File("aggregateMultiple");
        runTestAndStoreResult(spec, temp);
    }
    @Test
    public void singleKeySum2PreClusterGroupTest() throws Exception {
        JobSpecification spec = new JobSpecification();

        FileScanOperatorDescriptor csvScanner = new FileScanOperatorDescriptor(spec, splitProvider2, tupleParserFactory2,
                desc2);
        ...
        spec.addRoot(printer);
        runTest(spec);
        File temp=new File("aggregateMultiple2");
        runTestAndStoreResult(spec, temp);
    }
    ...
    public void singleKeyAvgPreClusterGroupTest() throws Exception {
        ...
        spec.addRoot(printer);
        runTest(spec);
        File temp=new File("singleKeyPreCluster");
        runTestAndStoreResult(spec, temp);
    }
    @Test
    public void singleKeyAv2g2PreClusterGroupTest() throws Exception {
        ...
        System.out.println("it run " + (endTime - startTime)
                + " 。");
        File temp=new File("singleKeyPr2eCluster");
        runTestAndStoreResult(spec, temp);
